...
class TypeGenerator:
    ...
    def __init__(self, implementation: ImplementationModel):
        ...
        self.style = cfile.StyleOptions(
            break_before_braces=cfile.BreakBeforeBraces.ATTACH,
        ...
        for root_node in self.implementation.gen_type_dependency_trees():
            ...
            for node in self.implementation.get_type_creation_order(root_node):
                ...
                if data_type.ref not in instance_refs:
                    ...
                    if (
                        isinstance(data_type, rte_element.BaseType)
                        and not data_type.native_declaration
                    ):
                        continue
                    # Ignore ImplementationTypes with non-RTE type emitter
                    if isinstance(data_type, rte_element.ImplementationType) and (
                        data_type.type_emitter is not None
                        and data_type.type_emitter.upper() != "RTE"
                    ):
                        continue
    ...
    def _gen_type_defs(
        self, data_types: Iterator[rte_element.Element]
    ) -> cfile.core.Sequence:
        """
        ...
        for data_type in data_types:
            if isinstance(data_type, rte_element.BaseType):
                code.append(
                    C.statement(C.typedef(data_type.name, data_type.native_declaration))
                )
            elif isinstance(data_type, rte_element.ScalarType):
                code.append(
                    C.statement(C.typedef(data_type.name, data_type.base_type.name))
                )
            elif isinstance(data_type, rte_element.RefType):
                code.append(
                    C.statement(C.typedef(data_type.name, data_type.impl_type.name))
                )
    ...
    def _gen_array_type_def(
        self, data_type: rte_element.ArrayType
    ) -> cfile.core.Statement:
        ...
        return C.statement(
            C.typedef(data_type.name, target_type, array=last_element.array_size)
        )

    def _gen_record_type_def(
        self, data_type: rte_element.RecordType
    ) -> cfile.core.Statement:
        members = []
