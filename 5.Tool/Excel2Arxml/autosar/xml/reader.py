...
ValueSpeficationElement = Union[
    ar_element.TextValueSpecification,
...
class Reader:
    ...
    def __init__(
        ...
        schema_version: int = ar_base.DEFAULT_SCHEMA_VERSION,
    ) -> None:
    ...
    def read_str_elem(
        self, xml: str, type_name: str | None = None
    ) -> None | ar_element.ARObject:
        """
        ...
        if xml_elem.tag not in self.observed_unsupported_elements:
            ...
            if self.warn_on_unprocessed_element:
                if self.file_path is not None:
                    file = (
                        self.file_path
                        if self.use_full_path_on_warning
                        else self.file_base_name
                    )
                    print(
                        f"{file}({xml_elem.sourceline}): Unprocessed element <{xml_elem.tag}>",
                        file=sys.stderr,
                    )
                else:
    ...
    def _read_referrable(self, element_map: ChildElementMap, data: dict) -> None:
        ...
        if short_name is not None:
            ...
            raise ar_exception.ParseError("Missing required element SHORT-NAME")
    ...
    def _read_multi_language_referrable(
        self, element_map: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_identifiable(
        self, element_map: ChildElementMap, attr: dict, data: dict
    ) -> None:
        """
    ...
    def _read_admin_data(
        self, xml_element: ElementTree.Element
    ) -> ar_element.AdminData:
        """
    ...
    def _read_schema_version(self) -> None:
        for key in self.xml_root.attrib.keys():
            if key.endswith("schemaLocation"):
                match = re.search(r"AUTOSAR_(\d+).xsd", self.xml_root.attrib[key])
    ...
    def _read_package_group(
        self, element_map: ChildElementMap, package: ar_element.Package
    ) -> None:
        """
    ...
    def _read_package_elements(
        self, package: ar_element.Package, xml_elements: ElementTree.Element
    ) -> None:
        """
        ...
        for xml_child_elem in xml_elements.findall("./*"):
            read_method = self.switcher_collectable.get(xml_child_elem.tag, None)
            if read_method is not None:
    ...
    def _read_sub_packages(
        self, package: ar_element.Package, xml_packages: ElementTree.Element
    ) -> None:
        """
        Reads AR:AR-PACKAGE.ELEMENTS
        Type: Utility
    ...
    def _read_annotations(
        self, xml_elem: ElementTree.Element
    ) -> list[ar_element.Annotation]:
        """
    ...
    def _read_general_annotation(
        self, xml_elem: ElementTree.Element, data: dict
    ) -> None:
        """
    ...
    def _read_break(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.Break:  # pylint: disable=unused-argument
        """
    ...
    def _read_documentation_block(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.DocumentationBlock:
        """
        ...
        for xml_child_elem in xml_elem.findall("./*"):
            if xml_child_elem.tag == "P":
                elem.append(self._read_multi_language_paragraph(xml_child_elem))
    ...
    def _read_emphasis_text(self, elem: ElementTree.Element) -> ar_element.EmphasisText:
        ...
        if len(elem.findall("./*")) > 0:
            raise NotImplementedError("Nesting inside <E> elements not yet supported")
    ...
    def _read_technical_term(
        self, elem: ElementTree.Element
    ) -> ar_element.TechnicalTerm:
        """
        ...
        if len(elem.findall("./*")) > 0:
            self._raise_parse_error(elem, 'Child elements in "AR:TT" not allowed')
        data["text"] = elem.text
    ...
    def _read_multi_language_long_name(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.MultilanguageLongName:
        """
    ...
    def _read_language_long_name(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.LanguageLongName:
        """
    ...
    def _read_language_specific_attr(self, attrib: dict, data: dict) -> None:
        ...
        data["language"] = ar_enum.xml_to_enum(
            "Language", attrib["L"]
        )  # L is a mandatory attribute

    def _read_mixed_content_for_long_name(
        self, xml_elem: ElementTree.Element, elem: ar_element.LanguageLongName
    ) -> None:
        """
    ...
    def _read_multi_language_overview_paragraph(
        self, xml_elem: ElementTree.Element
        ...
        for xml_child_elem in xml_elem.findall("./*"):
            if xml_child_elem.tag == "L-2":
                elem.append(self._read_language_overview_paragraph(xml_child_elem))
        return elem

    def _read_language_overview_paragraph(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.LanguageOverviewParagraph:
        """
    ...
    def _read_mixed_content_for_overview_paragraph(
        self, xml_elem: ElementTree.Element, elem: ar_element.LanguageOverviewParagraph
    ) -> None:
        """
    ...
    def _read_language_paragraph(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.LanguageParagraph:
        """
    ...
    def _read_language_verbatim(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.LanguageVerbatim:
        """
    ...
    def _read_mixed_content_for_paragraph(
        self, xml_elem: ElementTree.Element, elem: ar_element.LanguageParagraph
    ) -> None:
        """
    ...
    def _read_mixed_content_for_verbatim(
        self, xml_elem: ElementTree.Element, elem: ar_element.LanguageVerbatim
    ) -> None:
        """
    ...
    def _read_multi_language_paragraph(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.MultiLanguageParagraph:
        """
    ...
    def _read_paginateable_attrib(self, attrib: dict, data: dict) -> None:
        ...
        """
        if "BREAK" in attrib:
            data["page_break"] = ar_enum.xml_to_enum("PageBreak", attrib["BREAK"])
    ...
    def _read_multi_language_verbatim(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.MultiLanguageVerbatim:
        """
    ...
    def _read_multi_language_verbatim_attrib(self, attrib: dict, data: dict) -> None:
        ...
        if "HELP-ENTRY" in attrib:
            data["help_entry"] = str(attrib["HELP-ENTRY"])
        if "PGWIDE" in attrib:
            data["page_wide"] = ar_enum.xml_to_enum("PageWide", attrib["PGWIDE"])

    def _read_single_language_unit_names(
        self, xml_element: ElementTree.Element
    ) -> ar_element.SingleLanguageUnitNames:
        """
    ...
    def _read_mixed_content_for_unit_names(
        self, xml_element: ElementTree.Element, elem: ar_element.SingleLanguageUnitNames
    ) -> None:
        """
    ...
    def _read_compu_method(
        self, xml_element: ElementTree.Element
    ) -> ar_element.CompuMethod:
        """
    ...
    def _read_compu_method_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_computation(
        self, xml_element: ElementTree.Element
    ) -> ar_element.Computation:
        """
    ...
    def _read_compu_scales(
        self, xml_element: ElementTree.Element
    ) -> list[ar_element.CompuScale]:
        """
    ...
    def _read_compu_scale(
        ...
        if (compu_const_xml is not None) and (compu_rational_xml is not None):
            raise ar_exception.ParseError(
                "Can't define both COMPU-CONST and COMPU-RATIONAL-COEFFS"
    ...
    def _read_limit(
        self, xml_element: ElementTree.Element
    ) -> tuple[int | float, ar_enum.IntervalType]:
    ...
    def _read_compu_const(
        self, xml_element: ElementTree.Element
    ) -> ar_element.CompuConst:
        """
        ...
        if (v_xml is not None) and (vt_xml is not None):
            raise ar_exception.ParseError(
                "Can't define both V and VT" "in the same COMPU-CONST element"
            )
    ...
    def _read_compu_rational(
        self, xml_element: ElementTree.Element
    ) -> ar_element.CompuRational:
        """
        ...
        if xml_child is not None:
            data["denominator"] = self._read_compu_numerator_denominator_values(
                xml_child
            )
        self._report_unprocessed_elements(child_elements)
        return ar_element.CompuRational(**data)

    def _read_compu_numerator_denominator_values(
        self, xml_element: ElementTree.Element
    ) -> tuple:
        """
    ...
    def _read_data_constraint_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_data_constraint_rule(
        self, xml_element: ElementTree.Element
    ) -> ar_element.DataConstraintRule:
        """
    ...
    def _read_internal_constraint(
        self, xml_element: ElementTree.Element
    ) -> ar_element.InternalConstraint:
        """
        Writes complex type AR:INTERNAL-CONSTRS
        Type: Concrete
        Tag variants: 'INTERNAL-CONSTRS'
    ...
    def _read_physical_constraint(
        self, xml_element: ElementTree.Element
    ) -> ar_element.PhysicalConstraint:
        """
    ...
    def _read_constraint_base(
        self, child_elements: ChildElementMap, data: dict[str, Any]
    ) -> None:
        """
    ...
    def _read_scale_constraints(
        self, xml_element: ElementTree.Element
    ) -> list[ar_element.ScaleConstraint]:
        """
        ...
        for xml_child in xml_element.findall("./SCALE-CONSTR"):
            props_conditional = self._read_scale_constraint(xml_child)
            scale_constrs.append(props_conditional)
        return scale_constrs

    def _read_scale_constraint(
        self, xml_element: ElementTree.Element
    ) -> ar_element.ScaleConstraint:
        """
    ...
    def _read_unit_group(self, child_elements: ChildElementMap, data: dict) -> None:
        ...
        if xml_child is not None:
            data["physical_dimension_ref"] = self._read_physical_dimension_ref(
                xml_child
            )

    # Data type elements

    def _read_sw_addr_method(
        self, xml_element: ElementTree.Element
    ) -> ar_element.SwAddrMethod:
        """
    ...
    def _read_sw_addr_method_group(
        self, child_elements: ChildElementMap, _: dict
    ) -> None:
        """
        Reads group AR:SW-ADDR-METHOD
        Type: Utility
    ...
    def _read_sw_base_type(
        self, xml_element: ElementTree.Element
    ) -> ar_element.SwBaseType:
        """
    ...
    def _read_base_type(self, child_elements: ChildElementMap, data: dict) -> None:
        ...
        if xml_child is not None:
            data["byte_order"] = ar_enum.xml_to_enum("ByteOrder", xml_child.text)
        xml_child = child_elements.get("NATIVE-DECLARATION")
        if xml_child is not None:
            data["native_declaration"] = str(xml_child.text)

    def _read_sw_bit_representation(
        self, xml_element: ElementTree.Element
    ) -> ar_element.SwBitRepresentation:
        """
    ...
    def _read_sw_data_def_props(
        self, xml_element: ElementTree.Element
    ) -> ar_element.SwDataDefProps:
        """
    ...
    def _read_sw_data_def_props_variants(
        self, xml_elem: ElementTree.Element
    ) -> list[ar_element.SwDataDefPropsConditional]:
        """
        ...
        for xml_child in xml_elem.findall("./SW-DATA-DEF-PROPS-CONDITIONAL"):
            props_conditional = self._read_sw_data_def_props_conditional(xml_child)
            variants.append(props_conditional)
        return variants

    def _read_sw_data_def_props_conditional(
        self, xml_elem: ElementTree.Element
    ...
    def _read_sw_data_def_props_content(
        self, child_elements: ChildElementMap, data: dict[str, Any]
    ) -> None:
        """
        ...
        if xml_child is not None:
            data["calibration_access"] = ar_enum.xml_to_enum(
                "SwCalibrationAccess", xml_child.text
            )
    ...
    def _read_sw_text_props(
        ...
        if xml_child is not None:
            data["array_size_semantics"] = ar_enum.xml_to_enum(
                "ArraySizeSemantics", xml_child.text
            )
    ...
    def _read_sw_pointer_target_props(
        self, xml_element: ElementTree.Element
    ) -> ar_element.SwPointerTargetProps:
        """
        ...
        if xml_child is not None:
            data["function_ptr_signature_ref"] = self._read_function_ptr_signature_ref(
                xml_child
            )
        self._report_unprocessed_elements(child_elements)
        return ar_element.SwPointerTargetProps(**data)

    def _read_symbol_props(
        self, xml_element: ElementTree.Element
    ) -> ar_element.SymbolProps:
        """
    ...
    def _read_implementation_props(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_implementation_data_type_element(
        self, xml_element: ElementTree.Element
    ) -> ar_element.ImplementationDataTypeElement:  # noqa E128
        """
    ...
    def _read_implementation_data_type_element_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
        ...
        if xml_child is not None:
            data["array_impl_policy"] = ar_enum.xml_to_enum(
                "ArrayImplPolicy", xml_child.text
            )
        ...
        if xml_child is not None:
            data["array_size_handling"] = ar_enum.xml_to_enum(
                "ArraySizeHandling", xml_child.text
            )
        xml_child = child_elements.get("ARRAY-SIZE-SEMANTICS")
        if xml_child is not None:
            data["array_size_semantics"] = ar_enum.xml_to_enum(
        ...
        if xml_child is not None:
            sub_elements = []
            for sub_element_xml in xml_child.findall(
                "./IMPLEMENTATION-DATA-TYPE-ELEMENT"
            ):
                sub_elements.append(
                    self._read_implementation_data_type_element(sub_element_xml)
                )
    ...
    def _read_implementation_data_type(
        self, xml_element: ElementTree.Element
    ...
    def _read_implementation_data_type_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_application_primitive_data_type(
        self, xml_element: ElementTree.Element
    ) -> ar_element.ApplicationPrimitiveDataType:
        """
    ...
    def _read_application_array_element(
        self, xml_element: ElementTree.Element
    ) -> ar_element.ApplicationArrayElement:
        """
    ...
    def _read_application_array_element_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
        Reads complex type AR:APPLICATION-ARRAY-ELEMENT
    ...
    def _read_application_record_element(
        self, xml_element: ElementTree.Element
    ...
    def _read_application_record_element_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
        Reads complex type AR:APPLICATION-RECORD-ELEMENT
    ...
    def _read_application_array_data_type(
        self, xml_element: ElementTree.Element
    ) -> ar_element.ApplicationArrayDataType:
        """
    ...
    def _read_application_array_data_type_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_application_record_data_type(
        self, xml_element: ElementTree.Element
    ) -> ar_element.ApplicationRecordDataType:
        """
    ...
    def _read_application_record_data_type_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
        ...
        if xml_child is not None:
            ...
            for xml_record_element in xml_child.findall("./APPLICATION-RECORD-ELEMENT"):
                elements.append(
                    self._read_application_record_element(xml_record_element)
                )
            data["elements"] = elements

    def _read_data_type_map(
        self, xml_element: ElementTree.Element
    ) -> ar_element.DataTypeMap:
        """
    ...
    def _read_data_type_mapping_set(
        self, xml_element: ElementTree.Element
    ) -> ar_element.DataTypeMappingSet:
        """
    ...
    def _read_read_data_type_mapping_set_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
        ...
        if xml_child is not None:
            ...
            for xml_data_type_map_element in xml_child.findall("./DATA-TYPE-MAP"):
                data_type_maps.append(
                    self._read_data_type_map(xml_data_type_map_element)
                )
            data["data_type_maps"] = data_type_maps

    def _read_value_list(
        self, xml_element: ElementTree.Element
    ) -> ar_element.ValueList:
        """
        ...
        for xml_value in xml_element.findall("./V"):
            ...
            if number.value_format in (
                ar_enum.ValueFormat.HEXADECIMAL,
    ...
    def _read_compu_method_ref(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.CompuMethodRef:
        """
        ...
        if data["dest"] != "COMPU-METHOD":
            raise ar_exception.ParseError(
                f"Invalid value for DEST. Expected 'COMPU-METHOD', got '{data['dest']}'"
            )
        return ar_element.CompuMethodRef(xml_elem.text)

    def _read_data_constraint_ref(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.DataConstraintRef:
        """
        ...
        if data["dest"] != "DATA-CONSTR":
            raise ar_exception.ParseError(
                f"Invalid value for DEST. Expected 'DATA-CONSTR', got '{data['dest']}'"
            )
        return ar_element.DataConstraintRef(xml_elem.text)

    def _read_function_ptr_signature_ref(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.FunctionPtrSignatureRef:
        """
        ...
        if data["dest"] != "BSW-MODULE-ENTRY":
            raise ar_exception.ParseError(
                f"Invalid value for DEST. Expected 'BSW-MODULE-ENTRY', got '{data['dest']}'"
            )
        return ar_element.FunctionPtrSignatureRef(xml_elem.text)

    def _read_impl_data_type_ref(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.ImplementationDataTypeRef:
        """
    ...
    def _read_unit_ref(self, xml_elem: ElementTree.Element):
        ...
        dest_enum = ar_enum.xml_to_enum(
            "IdentifiableSubTypes", dest_text, self.schema_version
        )
        if dest_enum != ar_enum.IdentifiableSubTypes.UNIT:
            self._raise_parse_error(
                xml_elem, f"Invalid DEST attribute '{dest_text}'. Expected 'UNIT'"
            )
        return ar_element.UnitRef(xml_elem.text)

    def _read_physical_dimension_ref(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.PhysicalDimensionRef:
        """
        ...
        if dest_enum != ar_enum.IdentifiableSubTypes.PHYSICAL_DIMENSION:
            self._raise_parse_error(
                xml_elem,
                f"Invalid DEST attribute '{dest_text}'. Expected 'PHYSICAL-DIMENSION'",
            )
        return ar_element.PhysicalDimensionRef(xml_elem.text)

    def _read_index_data_type_ref(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.IndexDataTypeRef:
        """
    ...
    def _read_application_data_type_ref(
        self, xml_elem: ElementTree.Element
    ) -> ar_element.ApplicationDataTypeRef:
        """
    ...
    def _read_text_value_specification(
        self, xml_element: ElementTree.Element
    ) -> ar_element.TextValueSpecification:
        """
    ...
    def _read_text_value_specification_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_numerical_value_specification(
        self, xml_element: ElementTree.Element
    ) -> ar_element.NumericalValueSpecification:
        """
    ...
    def _read_not_available_value_specification(
        self, xml_element: ElementTree.Element
    ) -> ar_element.NotAvailableValueSpecification:
        """
    ...
    def _read_not_available_value_specification_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_array_value_specification(
        self, xml_element: ElementTree.Element
    ) -> ar_element.ArrayValueSpecification:
        """
    ...
    def _read_array_value_specification_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_record_value_specification(
        self, xml_element: ElementTree.Element
    ) -> ar_element.RecordValueSpecification:
        """
    ...
    def _read_record_value_specification_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_value_specification_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_application_value_specification(
        self, xml_element: ElementTree.Element
    ) -> ar_element.ApplicationValueSpecification:
        """
    ...
    def _read_application_value_specification_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_value_specification_element(
        self, xml_element: ElementTree.Element
    ...
    def _read_constant_specification(
        self, xml_element: ElementTree.Element
    ) -> ar_element.ConstantSpecification:
        """
    ...
    def _read_constant_specification_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_constant_reference_group(
        self, child_elements: ChildElementMap, data: dict
    ) -> None:
        """
    ...
    def _read_sw_values(self, xml_element: ElementTree.Element) -> ar_element.SwValues:
        """
    ...
    def _read_sw_values_group(
        self, xml_child_list: list[ElementTree.Element], data: dict
    ) -> None:
        """
        ...
        for xml_child in xml_child_list:
            if xml_child.tag == "VT":
                ...
                print(
                    f"Unprocessed child element in VALUE-GROUP: <{xml_child.tag}>",
                    file=sys.stderr,
                )

    def _read_value_group(
        self, xml_element: ElementTree.Element
    ...
    def _read_sw_axis_cont(
        self, xml_element: ElementTree.Element
    ) -> ar_element.SwAxisCont:
        """
    ...
    def _read_sw_axis_cont_group(
        ...
        xml_child = child_elements.get("CATEGORY")
        if xml_child is not None:
            data["category"] = ar_enum.xml_to_enum(
                "CalibrationAxisCategory", xml_child.text
            )
        xml_child = child_elements.get("UNIT-REF")
        if xml_child is not None:
            data["unit_ref"] = self._read_unit_ref(xml_child)
    ...
    def _read_sw_value_cont(
        self, xml_element: ElementTree.Element
    ) -> ar_element.SwValueCont:
        """
    ...
    def _read_sender_receiver_interface_group(
        self, element_map: ChildElementMap, data: dict
    ) -> None:
        """
        Reads AR:PORTINTERFACE
    ...
    def _read_variable_data_prototype_elem(
        self, xml_elements: ChildElementMap, _: dict
    ) -> None:
