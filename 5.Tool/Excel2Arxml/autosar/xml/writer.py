...
class _XMLWriter:
    ...
    def _add_content(
        self, tag: str, content: str = "", attr: TupleList = None, inline: bool = False
    ):
        if attr:
            if content:
    ...
    def _format_float(self, value: float) -> str:
        ...
        if math.isnan(value):
            ...
            return (
                tmp.quantize(decimal.Decimal(1))
    ...
    def _format_numerical_value(self, number: ar_element.NumericalValue) -> str:
        if number.value_format in (
            ar_enum.ValueFormat.DEFAULT,
            ar_enum.ValueFormat.DECIMAL,
        ):
...
class Writer(_XMLWriter):
    ...
    def write_str(
        self, document: ar_document.Document, skip_root_attr: bool = True
    ) -> str:
        """
    ...
    def _collect_identifiable_attributes(
        self, elem: ar_element.Identifiable, attr: TupleList
    ):
    ...
    def _write_document(
        self, document: ar_document.Document, skip_root_attr: bool = False
    ):
    ...
    def _write_package_elements(self, package: ar_element.Package) -> None:
        ...
        for elem in package.elements:
            ...
            if write_method is not None:
                ...
                raise NotImplementedError(f"Package: Found no writer for {class_name}")
        self._leave_child()
    ...
    def _collect_emphasis_text_attributes(
        self, elem: ar_element.EmphasisText
    ) -> None | TupleList:
        attr: TupleList = []
    ...
    def _collect_technical_term_attributes(
        self, elem: ar_element.TechnicalTerm, attr: TupleList
    ):
        """
    ...
    def _write_multi_language_long_name(
        self, elem: ar_element.MultilanguageLongName, tag: str
    ) -> None:
        """
    ...
    def _collect_language_specific_attr(
        self, elem: ar_element.LanguageSpecific, attr: TupleList
    ) -> None:
        """
        Collects attributes from attributeGroup AR:LANGUAGE-SPECIFIC
        """
        attr.append(
            ("L", ar_enum.enum_to_xml(elem.language))
        )  # The L attribute is mandatory

    def _write_multi_language_overview_paragraph(
        self, elem: MultiLanguageOverviewParagraph, tag: str
    ) -> None:
        """
    ...
    def _write_multi_language_verbatim(
        self, elem: ar_element.MultiLanguageVerbatim
    ) -> None:
        """
    ...
    def _collect_multi_language_verbatim_attributes(
        self, elem: ar_element.MultiLanguageVerbatim, attr: TupleList
    ):
        """
    ...
    def _write_language_overview_paragraph(
        self, elem: ar_element.LanguageOverviewParagraph
    ) -> None:
        """
    ...
    def _write_multi_language_paragraph(
        self, elem: ar_element.MultiLanguageParagraph
    ) -> None:
        """
    ...
    def _collect_multi_language_paragraph_attributes(
        self, elem: ar_element.MultiLanguageParagraph, attr: TupleList
    ):
        """
    ...
    def _collect_document_view_selectable_attributes(
        self, elem: ar_element.DocumentViewSelectable, attr: TupleList
    ):
        """
    ...
    def _collect_paginateable_attributes(
        self, elem: ar_element.Paginateable, attr: TupleList
    ):
        """
    ...
    def _write_single_language_unit_names(
        self, elem: ar_element.SingleLanguageUnitNames, tag: str
    ) -> None:
        """
    ...
    def _write_limit(
        self, tag: str, limit: int | float, limit_type: ar_enum.IntervalType
    ):
    ...
    def _write_constraint_base(
        self, elem: ar_element.InternalConstraint | ar_element.PhysicalConstraint
    ) -> None:
        """
    ...
    def _write_base_type(self, elem: ar_element.SwBaseType) -> None:
        ...
        if elem.byte_order is not None:
            self._add_content("BYTE-ORDER", ar_enum.enum_to_xml(elem.byte_order))
        if elem.native_declaration is not None:
            self._add_content("NATIVE-DECLARATION", str(elem.native_declaration))

    def _write_sw_data_def_props(
        self, elem: ar_element.SwDataDefProps, tag: str
    ) -> None:
        """
    ...
    def _write_sw_data_def_props_conditional(
        self, elem: ar_element.SwDataDefPropsConditional
    ) -> None:
        """
    ...
    def _write_sw_data_def_props_content(
        self, elem: ar_element.SwDataDefPropsConditional
    ) -> None:
        """
        ...
        if elem.display_presentation is not None:
            self._add_content(
                "DISPLAY-PRESENTATION", ar_enum.enum_to_xml(elem.display_presentation)
            )
        ...
        if elem.calibration_access is not None:
            self._add_content(
                "SW-CALIBRATION-ACCESS", ar_enum.enum_to_xml(elem.calibration_access)
            )
        ...
        if elem.impl_policy is not None:
            self._add_content("SW-IMPL-POLICY", ar_enum.enum_to_xml(elem.impl_policy))
        ...
        if elem.intended_resolution is not None:
            self._add_content(
                "SW-INTENDED-RESOLUTION", self._format_number(elem.intended_resolution)
            )
        ...
        if elem.is_empty:
            ...
            if elem.array_size_semantics is not None:
                self._add_content(
                    ...
                    ar_enum.enum_to_xml(elem.array_size_semantics),
                )
    ...
    def _write_sw_pointer_target_props(
        self, elem: ar_element.SwPointerTargetProps
    ) -> None:
        """
        ...
        if elem.is_empty:
            ...
            if elem.sw_data_def_props is not None:
                self._write_sw_data_def_props(
                    elem.sw_data_def_props, "SW-DATA-DEF-PROPS"
                )
    ...
    def _write_implementation_data_type_element(
        self, elem: ar_element.ImplementationDataTypeElement
    ) -> None:
        """
    ...
    def __write_implementation_data_type_element_group(
        self, elem: ar_element.ImplementationDataTypeElement
    ) -> None:
        """
        ...
        if elem.array_impl_policy is not None:
            self._add_content(
                "ARRAY-IMPL-POLICY", ar_enum.enum_to_xml(elem.array_impl_policy)
            )
        if elem.array_size is not None:
            self._add_content("ARRAY-SIZE", str(elem.array_size))
        if elem.array_size_handling is not None:
            self._add_content(
                "ARRAY-SIZE-HANDLING", ar_enum.enum_to_xml(elem.array_size_handling)
            )
        if elem.array_size_semantics is not None:
            self._add_content(
                "ARRAY-SIZE-SEMANTICS", ar_enum.enum_to_xml(elem.array_size_semantics)
            )
    ...
    def _write_implementation_data_type(
        self, elem: ar_element.ImplementationDataType
    ) -> None:
        """
    ...
    # 补充实现了_write_sender_receiver_interface()
    def _write_sender_receiver_interface(
        self, elem: ar_element.SenderReceiverInterface
    ):
        assert isinstance(elem, ar_element.SenderReceiverInterface)
        self._add_child("SENDER-RECEIVER-INTERFACE")
        self._write_referrable(elem)
        self._add_content("IS-SERVICE", "false")
        self._add_child("DATA-ELEMENTS")
        self._add_child("VARIABLE-DATA-PROTOTYPE")
        self._add_content("SHORT-NAME", elem.data_name)
        self._add_content(
            "TYPE-TREF", elem.Idt_name, [("DEST", "IMPLEMENTATION-DATA-TYPE")]
        )
        self._leave_child()
        self._leave_child()
        self._leave_child()

    def _write_implementation_data_type_group(
        self, elem: ar_element.ImplementationDataType
    ) -> None:
        """
        ...
        if elem.dynamic_array_size_profile is not None:
            self._add_content(
                "DYNAMIC-ARRAY-SIZE-PROFILE", str(elem.dynamic_array_size_profile)
            )
    ...
    def _write_application_primitive_data_type(
        self, elem: ar_element.ApplicationPrimitiveDataType
    ) -> None:
        """
    ...
    def _write_application_composite_element_data_prototype(
        self, elem: ar_element.ApplicationCompositeElementDataPrototype
    ) -> None:
        """
    ...
    def _write_application_array_element(
        self, elem: ar_element.ApplicationArrayElement
    ) -> None:
        """
    ...
    def _write_application_array_element_group(
        self, elem: ar_element.ApplicationArrayElement
    ) -> None:
        """
    ...
    def _write_application_record_element(
        self, elem: ar_element.ApplicationRecordElement
    ) -> None:
        """
    ...
    def _write_application_record_element_group(
        self, elem: ar_element.ApplicationRecordElement
    ) -> None:
        """
    ...
    def _write_application_array_data_type(
        self, elem: ar_element.ApplicationArrayDataType
    ) -> None:
        """
    ...
    def _write_application_array_data_type_group(
        self, elem: ar_element.ApplicationArrayDataType
    ) -> None:
        """
    ...
    def _write_application_record_data_type(
        self, elem: ar_element.ApplicationRecordDataType
    ) -> None:
        """
    ...
    def _write_application_record_data_type_group(
        self, elem: ar_element.ApplicationRecordDataType
    ) -> None:
        """
    ...
    def _write_data_type_map(self, elem: ar_element.DataTypeMap) -> None:
        ...
        if elem.appl_data_type_ref is not None:
            self._write_application_data_type_ref(
                elem.appl_data_type_ref, "APPLICATION-DATA-TYPE-REF"
            )
        if elem.impl_data_type_ref is not None:
            self._write_impl_data_type_ref(elem.impl_data_type_ref)
        self._leave_child()
    ...
    def _collect_base_ref_attr(self, elem: ar_element.BaseRef, attr: TupleList) -> None:
    ...
    def _write_function_ptr_signature_ref(
        self, elem: ar_element.FunctionPtrSignatureRef
    ) -> None:
        """
    ...
    def _write_impl_data_type_ref(
        self, elem: ar_element.ImplementationDataTypeRef
    ) -> None:
        """
    ...
    def _write_physical_dimension_ref(
        self, elem: ar_element.PhysicalDimensionRef
    ) -> None:
        """
    ...
    def _write_application_data_type_ref(
        self, elem: ar_element.ApplicationDataTypeRef, tag: str
    ) -> None:
        """
    ...
    def _write_text_value_specification(
        self, elem: ar_element.TextValueSpecification
    ) -> None:
        """
    ...
    def _write_numerical_value_specification(
        self, elem: ar_element.NumericalValueSpecification
    ) -> None:
        """
    ...
    def _write_not_available_value_specification(
        self, elem: ar_element.NotAvailableValueSpecification
    ) -> None:
        """
        ...
        if elem.is_empty_with_ignore({"default_pattern_format"}):
            ...
            if elem.default_pattern is not None:
                self._add_content(
                    "DEFAULT-PATTERN", str(elem.default_pattern)
                )  # TODO support numerical formats
            self._leave_child()

    def _write_array_value_specification(
        self, elem: ar_element.ArrayValueSpecification
    ) -> None:
        """
    ...
    def _write_array_value_specification_group(
        self, elem: ar_element.ArrayValueSpecification
    ) -> None:
        """
    ...
    def _write_record_value_specification(
        self, elem: ar_element.RecordValueSpecification
    ) -> None:
        """
    ...
    def _write_record_value_specification_group(
        self, elem: ar_element.RecordValueSpecification
    ) -> None:
        """
    ...
    def _write_application_value_specification(
        self, elem: ar_element.ApplicationValueSpecification
    ) -> None:
        """
    ...
    def _write_application_specification_group(
        self, elem: ar_element.ApplicationValueSpecification
    ) -> None:
        """
    ...
    def _write_value_specification_group(
        self, elem: ar_element.ValueSpecification
    ) -> None:
        """
    ...
    def _write_constant_specification(
        self, elem: ar_element.ConstantSpecification
    ) -> None:
        """
    ...
    def _write_constant_specification_group(
        self, elem: ar_element.ConstantSpecification
    ) -> None:
        """
    ...
    def _write_sw_axis_cont_group(self, elem: ar_element.SwAxisCont) -> None:
        ...
        if elem.unit_display_name is not None:
            self._write_single_language_unit_names(
                elem.unit_display_name, "UNIT-DISPLAY-NAME"
            )
