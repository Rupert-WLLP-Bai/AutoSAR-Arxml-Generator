...
class Namespace:
    ...
    def __init__(
        self, name: str, package_map: dict[str, str], base_ref: str = None
    ) -> None:
        self.name = name
...
class Workspace:
    ...
    def get_package_ref_by_role(
        self, namespace_name: str, role: ar_enum.PackageRole | str
    ):
        """
        ...
        if not isinstance(role, (ar_enum.PackageRole, str)):
            raise TypeError(
                f"role must be of type PackageRole or str. Got {str(type(role))}"
            )
    ...
    def make_packages(
        self, *refs: list[str]
    ) -> list[ar_element.Package] | ar_element.Package:
        """
    ...
    def create_document(
        self, file_path: str, packages: str | list[str]
    ...
    def _apply_element_template(
        self, template: ar_template.ElementTemplate, kwargs: dict
    ):
        """
        ...
        * Make sure the element doesn't already exists in the package
        ...
        package_ref = self.get_package_ref_by_role(
            template.namespace_name, template.package_role
        )
    ...
    def _create_dependencies(
        self, dependencies: list[ar_template.TemplateBase]
    ) -> list[Any]:
        items = []
