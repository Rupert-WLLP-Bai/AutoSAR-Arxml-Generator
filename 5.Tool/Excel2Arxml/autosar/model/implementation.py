...
class ImplementationModel:
    ...
    def create_from_element(
        self, element: ar_element.ARElement, is_source: bool = True
    ) -> Any:
        """
        ...
        if str(elem_ref) not in self.data_types:
            ...
            if isinstance(element, ar_element.SwBaseType):
                ...
                raise NotImplementedError(
                    "ImplementationDataType::" + str(element.category)
                )
            else:
    ...
    def _create_base_type(self, element: ar_element.SwBaseType) -> rte_element.BaseType:
        ...
        """
        elem_ref = element.ref()
        element = rte_element.BaseType(
            str(elem_ref), element.name, element.native_declaration
        )
        self.data_types[str(elem_ref)] = element
        return element

    def _create_scalar_type_from_element(
        self, element: ar_element.ImplementationDataType
    ) -> rte_element.ScalarType:
        """
        ...
        scalar_type = rte_element.ScalarType(
            str(elem_ref),
            element.name,
            base_type,
            symbol,
    ...
    def _create_ref_type_from_element(
        self, element: ar_element.ImplementationDataType
    ) -> rte_element.ScalarType:
        """
    ...
    def _create_array_type_from_element(
        self, element: ar_element.ImplementationDataType
    ) -> rte_element.ArrayType:
        elem_ref = str(element.ref())
        symbol = None
        if element.symbol_props is not None:
            symbol = element.symbol_props.symbol
        data_type = rte_element.ArrayType(
            elem_ref,
            element.name,
            symbol_name=symbol,
            type_emitter=element.type_emitter,
        ...
        if last_element.category == "VALUE":
            sub_element = self._create_array_type_element_from_scalar_type(
                elem_ref, last_element
            )
        elif last_element.category == "TYPE_REFERENCE":
            sub_element = self._create_array_type_element_from_ref_type(
                elem_ref, last_element
            )
        else:
            raise NotImplementedError(last_element.category)
        if last_element.array_impl_policy is not None:
            sub_element.impl_policy = rte_enumeration.ArrayImplPolicy(
                last_element.array_impl_policy
            )
        if last_element.array_size_handling is not None:
            sub_element.size_handling = rte_enumeration.ArraySizeHandling(
                last_element.array_size_handling
            )
        if last_element.array_size_semantics is not None:
            sub_element.size_semantics = rte_enumeration.ArraySizeSemantics(
                last_element.array_size_semantics
            )
        ...
        for ar_sub_elem in reversed(element.sub_elements[:-1]):
            ...
            rte_sub_element = rte_element.ArrayTypeElement(
                xml_ref, ar_sub_elem.name, ar_sub_elem.array_size
            )
            data_type.sub_elements.insert(0, rte_sub_element)
        self.data_types[str(elem_ref)] = data_type
        return data_type

    def _create_array_type_element_from_scalar_type(
        self, parent_ref: str, element: ar_element.ImplementationDataTypeElement
    ) -> rte_element.ArrayTypeElement:
        element_ref = parent_ref + "/" + element.name
        sw_data_def_props = element.sw_data_def_props
        base_type = self.create_from_ref(sw_data_def_props[0].base_type_ref, False)
        scalar_type = rte_element.ScalarType(element_ref, element.name, base_type)
        assert isinstance(element.array_size, int)
        return rte_element.ArrayTypeElement(
            element_ref, element.name, element.array_size, scalar_type
        )

    def _create_array_type_element_from_ref_type(
        self, parent_ref: str, element: ar_element.ImplementationDataTypeElement
    ) -> rte_element.ArrayTypeElement:
        element_ref = parent_ref + "/" + element.name
        sw_data_def_props = element.sw_data_def_props
        ...
        assert isinstance(element.array_size, int)
        return rte_element.ArrayTypeElement(
            element_ref, element.name, element.array_size, ref_type
        )

    def _create_struct_type_from_element(
        self, element: ar_element.ImplementationDataType
    ) -> rte_element.RecordType:
        elem_ref = str(element.ref())
        symbol = None
        if element.symbol_props is not None:
            symbol = element.symbol_props.symbol
        data_type = rte_element.RecordType(
            elem_ref,
            element.name,
            symbol_name=symbol,
            type_emitter=element.type_emitter,
        ...
        for ar_sub_elem in element.sub_elements:
            if ar_sub_elem.category == "VALUE":
                sub_element = self._create_struct_type_element_from_scalar_type(
                    elem_ref, ar_sub_elem
                )
            elif ar_sub_elem.category == "TYPE_REFERENCE":
                sub_element = self._create_struct_type_element_from_ref_type(
                    elem_ref, ar_sub_elem
                )
            else:
    ...
    def _create_struct_type_element_from_scalar_type(
        self, parent_ref: str, element: ar_element.ImplementationDataTypeElement
    ) -> rte_element.ArrayTypeElement:
        element_ref = parent_ref + "/" + element.name
        sw_data_def_props = element.sw_data_def_props
        base_type = self.create_from_ref(sw_data_def_props[0].base_type_ref, False)
        scalar_type = rte_element.ScalarType(element_ref, element.name, base_type)
        return rte_element.RecordTypeElement(element_ref, element.name, scalar_type)

    def _create_struct_type_element_from_ref_type(
        self, parent_ref: str, element: ar_element.ImplementationDataTypeElement
    ) -> rte_element.ArrayTypeElement:
        element_ref = parent_ref + "/" + element.name
        sw_data_def_props = element.sw_data_def_props
        impl_type = self.create_from_ref(sw_data_def_props[0].impl_data_type_ref, False)
        ref_type = rte_element.RefType(element_ref, element.name, impl_type)
        return rte_element.RecordTypeElement(element_ref, element.name, ref_type)

    def _create_ptr_type_from_element(
        self, element: ar_element.ImplementationDataType
    ) -> rte_element.PointerType:
        elem_ref = str(element.ref())
        symbol = None
        if element.symbol_props is not None:
            symbol = element.symbol_props.symbol
        ptr_target_props: ar_element.SwPointerTargetProps = element.sw_data_def_props[
            0
        ].ptr_target_props
        if ptr_target_props.target_category == "VALUE":
            target_type = self.create_from_ref(
                ptr_target_props.sw_data_def_props[0].base_type_ref, False
            )
        elif ptr_target_props.target_category == "TYPE_REFERENCE":
            target_type = self.create_from_ref(
                ptr_target_props.sw_data_def_props[0].impl_data_type_ref, False
            )
        else:
            raise NotImplementedError
        data_type = rte_element.PointerType(
            elem_ref,
            element.name,
            target_type,
            symbol_name=symbol,
    ...
    def _create_tree_node(self, data_type: rte_element.Element) -> Node:
        ...
        if isinstance(data_type, rte_element.BaseType):
            ...
            self._process_array_or_record_element_tree_node(
                node, last_element.data_type
            )
    ...
    def _process_array_or_record_element_tree_node(
        self, node: Node, element: rte_element.Element
    ) -> None:
